    ...
    useEffect(() => {
        if (isTransitioning) {
            const timer = setTimeout(() => {
                ...
                if (currentIndex === 0) {
                    setCurrentIndex(concerts.length);
                }
            }, 500);

            return () => clearTimeout(timer);
        }
    }, [currentIndex, concerts.length, isTransitioning]);
                ...
                {concerts.map((concert, index) => (
                    ...
                    >
                        {hoveredIndex === index ? (
                            ...
                            <img src={concert.image} style={imgStyle} alt={`Slide ${index + 1}`} />
                        )}
                        <div style={overlayStyle}>
                            <div style={titleStyle}>{concert.Name}</div>
                            <div style={datePriceStyle}>
                                <span>Giá từ: {concert.price.toLocaleString()} VND</span>
                                <span>{concert.date}</span>
                            </div>
                            <button style={buttonStyle} onClick={() => handleDetailClick(concert.id)}>Xem chi tiết</button>
                        </div>
                    </div>
                ))}
            </div>
